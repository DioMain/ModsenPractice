...
const InputSelect: React.FC<InputSelectProps> = ({ className, style, children, onSelectionChanged }) => {
	const selectionChangedCallback = useCallback((evt: React.ChangeEvent<HTMLSelectElement>) => {
		if (onSelectionChanged != undefined)
			onSelectionChanged(evt.currentTarget.value);
	}, [onSelectionChanged]);

	return (
		<select className={`inputselect sawarabi-gothic-regular ${className}`} style={style}
